...
export const MOVIE_SORT_OPTIONS: SortOption[] = [
  ...
  { value: 'primary_release_date.asc&vote_count.gte=100', label: 'Release Date (Oldest)' },
...
export const MovieFilterBar: React.FC<MovieFilterBarProps> = ({
  genres,
  selectedGenres,
  onSelectGenres,
  selectedProvider,
  ...
  const [openDropdown, setOpenDropdown] = useState<'platform' | 'genre' | 'country' | 'year' | 'rating' | 'sort' | null>(null);
  ...
  const getActiveTriggerRef = (type: 'platform' | 'genre' | 'country' | 'year' | 'rating' | 'sort') => {
    switch (type) {
  ...
  const selectedYearObj = MOVIE_YEAR_OPTIONS.find((y) => y.value === selectedYear);
  ...
  const toggleDropdown = (
    type: 'platform' | 'genre' | 'country' | 'year' | 'rating' | 'sort',
      ...
      >
        <div className="max-w-7xl mx-auto flex flex-col gap-2">
          ...
          >
            ...
            >
              <SlidersHorizontal className={`w-3.5 h-3.5 ${selectedProvider || openDropdown === 'platform' ? 'text-white' : 'text-hbo-cyan'}`} />
              ...
              {openDropdown === 'platform' && (
                <div className="w-56 max-h-80 overflow-y-auto no-scrollbar space-y-1 p-1">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold text-left transition tv-focus-target cursor-pointer ${
                      !selectedProvider ? 'bg-hbo-purple text-white font-extrabold border border-hbo-purple-light/40 shadow-sm' : 'text-gray-300 hover:bg-white/10 hover:text-white'
                    }`}
                  >
