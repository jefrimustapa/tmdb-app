...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  // Listen for Cross-Origin Embed postMessage events (Native Injected Subframe Tracker, VidLink, PlayerJS, Plyr)
  useEffect(() => {
    const handlePostMessage = (e: MessageEvent) => {
      try {
        ...
        // 1. Native Subframe HTML Injected Tracker (Approach A - 100% provider coverage)
        if (data.type === 'TMDB_EMBED_PROGRESS') {
          const current = data.currentTime ?? 0;
          const dur = data.duration ?? 0;
          if (current > 0) {
            recordProgress(current, dur, data.ended === true);
          }
          return;
        }

        // 2. VidLink PLAYER_EVENT (event: 'timeupdate' | 'pause' | 'ended' | 'time')
        ...
        // 3. VidLink MEDIA_DATA dictionary
