            ...
            webView.setWebViewClient(new BridgeWebViewClient(this.bridge) {
                ...
                public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                    if (request != null && request.getUrl() != null) {
                        ...
                        // CineSrc Embed Player Playback Observer & Seek Bridge
                        if (lower.contains("cinesrc.st/embed") && !lower.contains("/_next/") && !lower.contains("/api/") && !lower.contains("/cdn-cgi/")) {
                            try {
                                URL url = new URL(rawUrl);
                                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                                conn.setRequestMethod(request.getMethod());
                                ...
                                conn.setRequestProperty("Origin", "https://cinesrc.st");
                                ...
                                if (mimeType != null && mimeType.contains("html") && statusCode < 400) {
                                    try {
                                        ...
                                        String injectScript = "<script id=\"tmdb-cinesrc-bridge\">\n" +
                                            "(function setupCineSrcBridge() {\n" +
                                            "  var attachedVideo = null;\n" +
                                            "  var initialSeekDone = false;\n" +
                                            "  function getInitialResumeTime() {\n" +
                                            "    try {\n" +
                                            "      var match = window.location.hash.match(/t=(\\d+(\\.\\d+)?)/) || window.location.search.match(/[?&](?:t|start|time)=(\\d+(\\.\\d+)?)/);\n" +
                                            "      if (match && match[1]) return parseFloat(match[1]);\n" +
                                            "    } catch (e) {}\n" +
                                            "    return 0;\n" +
                                            "  }\n" +
                                            "  function sendMsg(evt, cur, dur) {\n" +
                                            "    try {\n" +
                                            "      var payload = { type: 'cinesrc', channel: 'cinesrc', event: evt, currentTime: cur, duration: dur };\n" +
                                            "      if (window.parent && window.parent !== window) {\n" +
                                            "        window.parent.postMessage(payload, '*');\n" +
                                            "      }\n" +
                                            "      if (window.AndroidBridge && typeof window.AndroidBridge.onNativePlaybackState === 'function') {\n" +
                                            ...
                                            "    } catch (e) {}\n" +
                                            "  }\n" +
                                            "  function performSeek(v, target) {\n" +
                                            "    if (!v || isNaN(target) || target <= 0) return;\n" +
                                            "    try {\n" +
                                            ...
                                            "    attachedVideo = v;\n" +
                                            "    var resumeTime = getInitialResumeTime();\n" +
                                            "    if (resumeTime > 0 && !initialSeekDone) {\n" +
                                            "      if (v.readyState >= 1) {\n" +
                                            "        initialSeekDone = true;\n" +
                                            "        performSeek(v, resumeTime);\n" +
                                            "      }\n" +
                                            "    }\n" +
                                            "    v.addEventListener('loadedmetadata', function() {\n" +
                                            "      if (!initialSeekDone) {\n" +
                                            "        var rt = getInitialResumeTime();\n" +
                                            "        if (rt > 0) {\n" +
                                            "          initialSeekDone = true;\n" +
                                            "          performSeek(v, rt);\n" +
                                            "        }\n" +
                                            "      }\n" +
                                            "      sendMsg('loadedmetadata', v.currentTime, v.duration || 0);\n" +
                                            "    });\n" +
                                            "    v.addEventListener('timeupdate', function() {\n" +
                                            "      if (v.duration > 0 && v.currentTime > 0) {\n" +
                                            "        sendMsg('timeupdate', v.currentTime, v.duration);\n" +
                                            ...
                                            "      if (d && (d.type === 'seek' || d.type === 'SEEK' || d.event === 'seek') && (d.time !== undefined && d.time !== null)) {\n" +
                                            "        var target = parseFloat(d.time);\n" +
                                            "        var vid = attachedVideo || window.__cinesrcVideo || document.querySelector('video');\n" +
                                            "        if (vid && !isNaN(target) && target >= 0) {\n" +
                                            "          initialSeekDone = true;\n" +
                                            "          performSeek(vid, target);\n" +
                                            "        }\n" +
                                            "      }\n" +
                                            "    } catch (e) {}\n" +
                                            "  });\n" +
                                            "  setInterval(function() {\n" +
                                            "    var vid = window.__cinesrcVideo || document.querySelector('video');\n" +
                                            "    if (vid && vid !== attachedVideo) attach(vid);\n" +
                                            "  }, 400);\n" +
                                            "})();\n" +
                                            "</script>\n";
